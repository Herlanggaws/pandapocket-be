...
func getExpensesHandler(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for rows.Next() {
			...
			err := rows.Scan(
				&exp.ID, &exp.UserID, &exp.CategoryID, &exp.Amount, &exp.Description, &exp.Date, &exp.CreatedAt,
...
func createExpenseHandler(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Get user's primary currency from preferences
		...
		result, err := db.Exec(`
			...
			VALUES (?, ?, ?, ?, ?, ?)
		`, userID, req.CategoryID, primaryCurrencyID, req.Amount, req.Description, date)
		if err != nil {
		...
		`, expenseID).Scan(
			...
			&cat.ID, &catUserIDPtr, &cat.Name, &cat.Color, &cat.IsDefault, &cat.CategoryType, &cat.CreatedAt,
			&curr.ID, &curr.UserID, &curr.Code, &curr.Name, &curr.Symbol, &curr.IsDefault, &curr.CreatedAt,
		)
		if err != nil {
...
func updateBudgetHandler(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		userID := c.GetInt("user_id")
		...
		`, req.CategoryID, req.Amount, req.Period, startDate, endDate, budgetID, userID)
		if err != nil {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch updated budget"})
			return
		}
		cat.UserID = catUserIDPtr
		budget.Category = cat
